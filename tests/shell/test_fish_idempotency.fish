...
set -l count (fish -c "
    ...
    source '$integration'
    source '$integration'
    bind | string match -r '_termcmp_report_buffer' | count
:")

if test "$count" != "1"
    echo "FAIL [bind]: _termcmp_report_buffer binding appears $count times, expected 1" >&2
...
set -l prompt_handlers (fish -c "
    source '$integration'
    source '$integration'
    functions --handlers | string match -r '_termcmp_prompt' | count
:")
if test "$prompt_handlers" != "1"
    echo "FAIL [event]: _termcmp_prompt event handler count = $prompt_handlers, expected 1" >&2
    exit 1
end
...
set -l preexec_handlers (fish -c "
    source '$integration'
    source '$integration'
    functions --handlers | string match -r '_termcmp_preexec' | count
:")
if test "$preexec_handlers" != "1"
    echo "FAIL [event]: _termcmp_preexec event handler count = $preexec_handlers, expected 1" >&2
    exit 1
end
