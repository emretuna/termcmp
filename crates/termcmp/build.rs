...
fn main() {
    // Re-run whenever the revision changes. `.git/HEAD` alone holds only the
    // branch name (e.g. `ref: refs/heads/main`) and its mtime moves on branch
    // switches, not commits — so the version string went stale on every commit
    // on the same branch. Watch the ref tree too; each missing path is skipped
    // because Cargo treats a rerun-if-changed path that doesn't exist as
    // "always changed", which would re-run this script on every build in
    // cargo-dist tarballs / shallow clones where .git is absent.
