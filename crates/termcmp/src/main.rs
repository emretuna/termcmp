...
fn resolve_default_shell() -> OsString {
    ...
    let result = resolve_default_shell_from(|name| std::env::var_os(name));
    tracing::info!(
        shell_env = ?shell_var,
        resolved = %result.to_string_lossy(),
...
fn run_proxy(
    ...
    let (reader_stream, supervisor_stream) =
        std::os::unix::net::UnixStream::pair().context("failed to create UnixStream pair")?;

    let outer_fd = 0; // stdin is the outer tty
    let stream_fd = reader_stream.as_raw_fd();

    let reader_pid =
        pty::spawn::fork_reader(outer_fd, stream_fd).context("failed to fork reader")?;

    // Parent: drop reader_stream, keep supervisor_stream
    drop(reader_stream);
    supervisor_stream.set_nonblocking(true).ok();

    let rt = tokio::runtime::Runtime::new().context("failed to create tokio runtime")?;
    let exit_code = rt.block_on(pty::run_proxy(
        &shell,
        &args,
        &config,
        supervisor_stream,
