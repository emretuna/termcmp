...
# description_box = \"off\"  # \"off\" for inline descriptions, \"on\" for wrapped adjacent box
...
# small_terminal_threshold = 15  # Rows below which compact mode activates
# small_terminal_max_visible = 5  # Max visible suggestions in compact mode
# compact_mode = \"auto\"  # Small terminal handling: \"auto\" | \"always\" | \"never\"
# tui_process_blocklist = []  # Additional TUI process names that suppress popup (extends built-in blocklist for Go apps)
...
mod drift_tests {
    ...
    #[test]
    fn install_template_order_includes_zoxide() {
        let order_line = DEFAULT_CONFIG_TOML
            .lines()
            .find(|l| l.contains("order ="))
            .expect("template has an order line");
        assert!(
            order_line.contains("zoxide"),
            "order line must include zoxide between filesystem and commands: {order_line}"
        );
    }
}
