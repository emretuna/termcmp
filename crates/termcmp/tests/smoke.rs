...
fn test_popup_renders_and_dismisses_on_trigger() {
    ...
    // OSC 133;A (prompt start marker) must be emitted before OSC 7772 to set
    // in_prompt=true in the parser. Without this, the popup eligibility check
    // fails even though we have a valid command buffer. Real shell integration
    // emits OSC 133;A at the start of every prompt.
    let mut envelope = Vec::new();
    envelope.extend_from_slice(b"\x1b]133;A\x07"); // OSC 133;A - prompt start
    envelope.extend_from_slice(&build_osc7772_envelope("git ", 4));
    envelope.push(b'X');
...
fn test_popup_is_cleared_before_later_shell_output() {
    ...
    // Emit OSC 133;A to set in_prompt=true before sending OSC 7772
