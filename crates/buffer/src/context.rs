...
impl CommandContext {
    /// Returns the full command path (command + all args), useful for fuzzy matching
    /// against cached completions. Returns empty string if at command position.
    ///
    /// Example: for `git commit `, returns "git commit"
    pub fn full_command_path(&self) -> String {
        if self.word_index == 0 {
            return String::new();
        }
        let mut parts = Vec::new();
        if let Some(cmd) = &self.command {
            parts.push(cmd.as_str());
        }
