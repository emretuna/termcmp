...
///
/// `Unknown` exists so a stale parser observing a new reason code from a
/// newer shell integration does not silently drop the frame — downstream
/// consumers can still log the raw code and detail.
...
/// Operator-friendly colon-separated rendering (`<reason_code>:<detail>`).
/// Used by the `tracing::warn!` emission in `performer.rs` so the operator
/// sees `shell-side runtime diagnostic: env_truncated:65536` rather than the
...
pub struct TerminalState {
    ...
    /// proxy's stdout task to dismiss popups on prompt/command transitions.
    prompt_changed: bool,
    cwd_dirty: bool,
    ...
    /// effectively per-process (narrow 7772 alphabet prevents `;`/BEL/ESC splitting).
...
impl TerminalState {
    ...
    /// to a new binary. Idempotent after first call (strict buffer decode drops frame, lenient path decode best-effort).
    ...
    /// One-shot — repeated polls without an intervening dispatch yield `None`.
    ...
    /// this is a local prediction, not a shell-reported update via OSC 7770/7772.
    ...
    /// One-shot: reports whether the prompt state toggled since the last
    /// drain, then clears the flag. The proxy's stdout task uses this to
    /// dismiss any visible popup when a command starts executing or when
    /// the shell returns to a prompt.
    pub fn take_prompt_changed(&mut self) -> bool {
        let v = self.prompt_changed;
...
mod tests {
    ...
    fn diagnostic_display_renders_adr_format() {
        // Colon-separated operator-visible shape (`<reason_code>:<detail>`) so
