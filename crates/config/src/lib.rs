...
pub struct AiConfig {
    ...
    /// Redact secrets (API keys, tokens, passwords) from buffer and history
    /// before sending to LLM providers. Default `true`.
...
impl Default for AiConfig {
    fn default() -> Self {
        Self {
            completion: AiCompletionConfig::default(),
            ask: AiAskConfig::default(),
...
/// Compact mode for small terminal handling.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(rename_all = "lowercase")]
pub enum CompactMode {
    /// Enable small terminal logic when screen_rows < threshold.
    #[default]
    Auto,
    /// Always use compact mode regardless of screen size.
    Always,
    /// Never use compact mode.
    Never,
}
...
pub struct PopupConfig {
    ...
    /// during normalization. Default 40.
    #[serde(deserialize_with = "deserialize_saturating_u16")]
    pub min_width: u16,
    ...
    #[serde(deserialize_with = "deserialize_saturating_u16")]
    pub max_width: u16,
    /// Description box mode. When `"on"`, an adjacent box is rendered next
    ...
    /// rendering on `description_box == On`.
    ...
    pub nerd_icons: bool,
    /// Threshold (rows) below which compact mode activates. When `screen_rows`
    /// is less than this value, visible suggestions are capped and scrolling is
    /// disabled to prevent the prompt from going off-screen. Clamped to
    /// `[5, 50]` during normalization. Default 15.
    #[serde(deserialize_with = "deserialize_saturating_u16")]
    pub small_terminal_threshold: u16,
    /// Maximum visible suggestions in compact mode. `0` resets to default 5;
    /// values above 10 clamp to 10. Default 5.
    pub small_terminal_max_visible: usize,
    /// Compact mode for small terminals: `"auto"` enables small terminal logic,
    /// `"never"` disables, `"always"` forces compact mode. Default `"auto"`.
    pub compact_mode: CompactMode,
    /// Process names that suppress the popup even when at prompt.
    /// Go TUI apps (omp, pi agent, btm, etc.) don't enter alt screen and keep
    /// `in_prompt == true`, so this blocklist is the only gate. Default empty
    /// — the handler's `DEFAULT_TUI_BLOCKLIST` is always checked first, this
    /// extends it for advanced users.
    pub tui_process_blocklist: Vec<String>,
}
...
pub struct ProvidersConfig {
    pub commands: bool,
    pub filesystem: bool,
    /// Enable fish/zsh shell-native completion providers. Default true.
...
fn apply_overrides(overrides: &ThemeFile, base: ResolvedTheme) -> ResolvedTheme {
    ResolvedTheme {
        ...
        index_hints: overrides.index_hints.clone().unwrap_or(base.index_hints),
        key_hints: overrides.key_hints.clone().unwrap_or(base.key_hints),
    }
}
...
pub fn all_field_paths() -> Vec<&'static str> {
    vec![
        ...
        "popup.key_hints",
        "popup.nerd_icons",
        "popup.small_terminal_threshold",
        "popup.small_terminal_max_visible",
        "popup.compact_mode",
        "popup.tui_process_blocklist",
        // [suggest]
        "suggest.max_results",
        ...
        // [ai]
        "ai.redact_secrets",
        // [ai.providers.<name>] — map keys are dynamic so one representative
        // name stands in for drift coverage; bare-key match keeps both the
        // install-template and CONFIGURATION.md guards honest here.
        "ai.providers.example.base_url",
        "ai.providers.example.api_key",
        "ai.providers.example.api",
        "ai.providers.example.thinking_budget",
        "ai.providers.example.extra_body",
        "ai.providers.example.models",
        "ai.providers.example.models.max_tokens",
    ]
}
...
impl TermcmpConfig {
    ...
    pub fn normalize(&mut self) {
        ...
        // Small terminal compact mode clamps.
        if self.popup.small_terminal_threshold < 5 {
            tracing::warn!(
                "popup.small_terminal_threshold={} below floor 5, clamping",
                self.popup.small_terminal_threshold,
            );
            self.popup.small_terminal_threshold = 5;
        }
        if self.popup.small_terminal_threshold > 50 {
            tracing::warn!(
                "popup.small_terminal_threshold={} exceeds ceiling 50, clamping",
                self.popup.small_terminal_threshold,
            );
            self.popup.small_terminal_threshold = 50;
        }
        if self.popup.small_terminal_max_visible == 0 {
            tracing::warn!(
                "popup.small_terminal_max_visible=0 is invalid (would hide all suggestions), \
                 clamping to default 5",
            );
            self.popup.small_terminal_max_visible = 5;
        }
        if self.popup.small_terminal_max_visible > 10 {
            tracing::warn!(
                "popup.small_terminal_max_visible={} exceeds ceiling 10, clamping",
                self.popup.small_terminal_max_visible,
            );
            self.popup.small_terminal_max_visible = 10;
        }
        // AI feature sanity.
        fn clamp_feature(
            feature: &str,
...
"#;
    ...
    fn test_resolve_no_preset_uses_dark() {
        ...
        assert_eq!(resolved.index_hints, "dim");
        assert_eq!(resolved.key_hints, "dim");
    }
    ...
    fn test_all_builtin_themes_have_required_fields() {
        ...
        for name in builtins {
            ...
            let required: &[(&str, &str)] = &[
                ...
                ("index_hints", &resolved.index_hints),
                ("key_hints", &resolved.key_hints),
            ];
    ...
    #[test]
    fn test_resolve_custom_hint_styles_used_verbatim() {
        let dir = tempfile::TempDir::new().unwrap();
        let themes = dir.path().join("themes");
        std::fs::create_dir_all(&themes).unwrap();
        std::fs::write(
            themes.join("hinted.toml"),
            "index_hints = \"bold\"\nkey_hints = \"fg:196\"\n",
        )
        .unwrap();
        ...
        let resolved = config.resolve(Some(dir.path())).unwrap();
        assert_eq!(resolved.index_hints, "bold");
        assert_eq!(resolved.key_hints, "fg:196");
    }
...
"#;
        let config: TermcmpConfig = toml::from_str(toml_str).unwrap();
        assert_eq!(config.popup.description_box, DescriptionBoxMode::On);
    }
...
"#;
    ...
    #[test]
    fn normalize_clamps_small_terminal_threshold() {
        let mut cfg = TermcmpConfig::default();
        cfg.popup.small_terminal_threshold = 3;
        cfg.normalize();
        assert_eq!(cfg.popup.small_terminal_threshold, 5);

        cfg.popup.small_terminal_threshold = 60;
        cfg.normalize();
        assert_eq!(cfg.popup.small_terminal_threshold, 50);
    }

    #[test]
    fn normalize_clamps_small_terminal_max_visible() {
        let mut cfg = TermcmpConfig::default();
        cfg.popup.small_terminal_max_visible = 0;
        cfg.normalize();
        assert_eq!(cfg.popup.small_terminal_max_visible, 5);

        cfg.popup.small_terminal_max_visible = 15;
        cfg.normalize();
        assert_eq!(cfg.popup.small_terminal_max_visible, 10);
    }
}
