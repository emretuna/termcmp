...
/// Construct a parser positioned at a prompt marker. The popup eligibility
/// contract intentionally rejects unmarked terminal state.
fn make_parser() -> Arc<Mutex<parser::TerminalParser>> {
    let mut parser = parser::TerminalParser::new(24, 80);
...
async fn fast_async_with_typed_query_refilters_pool() {
    ...
    let async_items = vec![
        ...
        Suggestion {
            text: "git checkout feature-x".into(),
            kind: SuggestionKind::Subcommand,
            source: SuggestionSource::Provider,
            ..Default::default()
        },
    ...
    let texts: Vec<String> = handler
        ...
        .collect();
    assert!(
        texts.contains(&"git checkout main".to_string()),
    ...
    assert!(
        !texts.contains(&"git checkout --zzz".to_string()),
        "flag '--zzz' must be filtered out by the 'main' query, got: {texts:?}"
    );
    assert!(
        !texts.contains(&"git checkout feature-x".to_string()),
