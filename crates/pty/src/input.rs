...
mod tests {
    ...
    #[test]
    fn arrow_keys_round_trip_byte_identical() {
        // Parser -> re-encode must preserve the exact on-wire bytes so SS3
        // sequences (application cursor-keys mode) are not rewritten to CSI.
        for (bytes, key) in [
            (b"\x1BOA".to_vec(), KeyEvent::ArrowUpSs3),
            (b"\x1BOB".to_vec(), KeyEvent::ArrowDownSs3),
            (b"\x1BOC".to_vec(), KeyEvent::ArrowRightSs3),
            (b"\x1BOD".to_vec(), KeyEvent::ArrowLeftSs3),
            (b"\x1B[A".to_vec(), KeyEvent::ArrowUp),
            (b"\x1B[B".to_vec(), KeyEvent::ArrowDown),
            (b"\x1B[C".to_vec(), KeyEvent::ArrowRight),
            (b"\x1B[D".to_vec(), KeyEvent::ArrowLeft),
        ] {
            assert_eq!(parse_keys(&bytes), vec![key.clone()]);
            assert_eq!(crate::handler::key_to_bytes(&key), bytes);
        }
    }
