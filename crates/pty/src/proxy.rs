...
use crate::spawn::{open_pty_pair, spawn_shell_with_pty, PtyMaster};
...
fn detect_shell_kind(shell: &OsStr) -> ShellKind {
    let name = std::path::Path::new(shell)
        ...
        .and_then(|n| n.to_str())
        .unwrap_or("");
    // Strip the leading '-' that login(1) prepends to the shell's argv[0]
    // (e.g. "-zsh"); `file_name()` preserves it, which would misidentify the
    // shell as Other and disable OSC 7772 buffer reporting.
...
pub async fn run_proxy(
    ...
    // Wrap master_fd for I/O
    let inner_master_fd = std::os::fd::AsRawFd::as_raw_fd(&master_fd);
    let master = PtyMaster::new(master_fd);
    ...
    // TUI detection is 100% signal-based (DECSET 1049 alt screen + OSC 133 prompt markers).
    // No multiplexer process-tree heuristic — the parser tracks alt screen directly.
    ...
    let theme = PopupTheme {
        ...
        index_hints_on: parse_style(&resolved_theme.index_hints)
            .context("invalid theme.index_hints style")?,
        key_hints_on: parse_style(&resolved_theme.key_hints)
            .context("invalid theme.key_hints style")?,
    ...
    let handler = {
        let mut h = InputHandler::new(terminal_profile, shell_kind)
            ...
            .with_small_terminal_threshold(config.popup.small_terminal_threshold)
            .with_small_terminal_max_visible(config.popup.small_terminal_max_visible)
            .with_compact_mode(config.popup.compact_mode)
            ...
            .with_tui_process_blocklist(config.popup.tui_process_blocklist.clone())
            .with_suggest_config(
    ...
    // Task A: reader_stream → PTY (user keystrokes to shell, with popup interception)
    ...
    let stdin_handle = tokio::task::spawn_blocking(move || {
        ...
        'stdin: loop {
            let n = match reader_stream.read(&mut buf) {
                Ok(0) => break,
                Ok(n) => n,
                    ...
                    crate::handler::KeyOutcome::Forward(forward) => {
                        if !forward.is_empty() {
                            ...
                            if has_isig {
                                ...
                                let mut filtered = Vec::with_capacity(forward.len());
                                for &byte in &forward {
                                    if byte == termios.c_cc[libc::VINTR] {
                                        // Send SIGINT to foreground pgrp
                                        let fg_pgrp = unsafe { libc::tcgetpgrp(outer_fd) };
                                        if fg_pgrp > 0 {
                                            unsafe { libc::kill(-fg_pgrp, libc::SIGINT) };
                                        }
                                    } else if byte == termios.c_cc[libc::VQUIT] {
                                        let fg_pgrp = unsafe { libc::tcgetpgrp(outer_fd) };
                                        if fg_pgrp > 0 {
                                            unsafe { libc::kill(-fg_pgrp, libc::SIGQUIT) };
                                        }
                                    } else if byte == termios.c_cc[libc::VSUSP] {
                                        let fg_pgrp = unsafe { libc::tcgetpgrp(outer_fd) };
                                        if fg_pgrp > 0 {
                                            unsafe { libc::kill(-fg_pgrp, libc::SIGTSTP) };
                                        }
                                    } else {
                                        // Not a signal character, forward it
                                        filtered.push(byte);
                                    }
                                }

                                // Forward non-signal bytes
                                if !filtered.is_empty() {
                                    match pty_writer.lock() {
                                        ...
                                        Err(e) => {
                                            tracing::warn!(
                                                "pty_writer mutex poisoned in stdin task: {e}"
                                            );
                                            break 'stdin;
                                        }
                                    }
    ...
    let stdout_handle = tokio::task::spawn_blocking(move || {
        ...
        loop {
            ...
            // Drain prompt_changed: if the shell transitioned between prompt
            // and command execution (OSC 133;A/C or 7771;A/C), dismiss any
            // visible popup. This catches TUI apps that don't use the alternate
                ...
                if prompt_changed {
                    ...
                    let cleanup_ticket = {
                        let mut h = match handler_for_stdout.lock() {
                            ...
                            Err(e) => {
                                tracing::warn!(
                                    "handler mutex poisoned in stdout task (prompt): {e}"
                                );
                                break;
                            }
                        };
                        // dismiss() no-ops when no popup is visible, so it is
                        // safe on both prompt→command and command→prompt transitions.
                        // dismiss() no-ops when no popup is visible, so it is
                        // safe on both prompt→command and command→prompt transitions.
                        h.dismiss(&mut cleanup);
                        // Invalidate in-flight async generations so results spawned
                        // before the transition cannot render after it completes.
                    ...
                    if !cleanup.is_empty() {
                        if let Err(e) =
                            write_overlay_if_current(&handler_for_stdout, cleanup_ticket, &cleanup)
                        {
                            tracing::debug!("Task B prompt-change cleanup write failed: {e}");
                            break;
                        }
                    }
                }
            ...
            let (buffer_dirty, buffer_pending_display) = {
                match parser_for_stdout.lock() {
                    Ok(mut p) => {
                        let state = p.state_mut();
                        let dirty = state.take_buffer_dirty();
                        let pending = state.buffer_pending_display();
                        (dirty, pending)
                    }
                    Err(e) => {
            ...
            if buffer_dirty {
                ...
                let render_ticket = {
                    ...
                    let auto_enabled = h.auto_trigger_enabled();
                    let debounce_suppressed = h.is_debounce_suppressed();
            ...
            let (cwd_dirty, shell_env_dirty, new_cwd) = {
                match parser_for_stdout.lock() {
                    Ok(mut p) => {
                        let state = p.state_mut();
                        let cwd_dirty = state.take_cwd_dirty();
                        let new_cwd = if cwd_dirty {
                            state.cwd().cloned()
                        } else {
                            None
                        };
                        (cwd_dirty, state.take_shell_env_dirty(), new_cwd)
                    }
                    Err(e) => {
                        tracing::warn!("parser mutex poisoned in stdout task: {e}");
                        break;
            ...
            // Mirror the inner shell's CWD onto the proxy's own process
            // directory. Multiplexers like herdr (`new_cwd = "follow"`)
            // derive a new split's CWD from the pane's direct child process;
            // under termcmp that child is the proxy, so without this every
            // split inherits the directory where termcmp launched instead of
            // the shell's current one. The inner shell keeps its own cwd on
            // its PTY — this only changes what the outside world observes.
            if let Some(dir) = new_cwd {
                if let Err(e) = std::env::set_current_dir(&dir) {
                    tracing::debug!("failed to sync proxy cwd to {dir:?}: {e}");
                }
            }
    ...
    // Supervisor ignores SIGINT/SIGQUIT/SIGTSTP — these are synthesized to
    // children by Task A via kill(). Out-of-band signals to the supervisor
    // are dropped; only SIGTERM/SIGHUP trigger shutdown.
    let mut sigchld = signal(SignalKind::child()).context("failed to register SIGCHLD handler")?;
    let mut mirror_tick = tokio::time::interval(Duration::from_millis(100));

    // Wait for either an I/O task to finish or a signal
    let mut signal_shutdown = false;
    // Set once the shell child has been reaped; stops mirror updates so the
    ...
    loop {
        tokio::select! {
            ...
            _ = sigchld.recv() => {
                // Reap zombies and update mirror
                unsafe {
                ...
                // Update foreground mirror — but never after the shell died:
            ...
            _ = mirror_tick.tick() => {
                ...
                // generated by the terminal and delivered to the outer tty's
                // fg pgrp — an inner process, not us — so we cannot rely on
                // receiving it; polling here is the only reliable path.
                match (get_terminal_size(), master.winsize()) {
                    (Ok(outer), Ok(inner)) if (outer.rows, outer.cols) != (inner.rows, inner.cols) => {
                        // Setting the inner master's winsize makes the kernel
                        // deliver SIGWINCH to the inner fg pgrp automatically.
                        if let Err(e) = master.resize(outer.rows, outer.cols) {
                            tracing::debug!("inner PTY size reconciliation failed: {e}");
                        }
                        // Update parser screen dimensions and dismiss a visible popup so a
                        // size change detected by polling (SIGWINCH is delivered to the inner
                        // fg pgrp, not us, under tmux/wrapped topologies) still re-anchors
                        // compact-mode geometry on the next trigger instead of rendering at a
                        // stale large size.
                        match parser.lock() {
                            Ok(mut p) => {
                                p.state_mut().update_dimensions(outer.rows, outer.cols);
                            }
                            Err(e) => {
                                tracing::warn!("parser mutex poisoned on mirror resize: {e}");
                            }
                        }
                        let mut render_buf = Vec::new();
                        let render_ticket = match handler.lock() {
                            ...
                            Err(e) => {
                                tracing::warn!("handler mutex poisoned on mirror resize: {e}");
                                None
                            }
                        };
                        if !render_buf.is_empty() {
                            ...
                            {
                                tracing::debug!("mirror resize overlay write/flush failed: {e}");
                                break;
                            }
                        }
                    }
                    (Err(e), _) | (_, Err(e)) => {
                        tracing::debug!("size read for reconciliation failed: {e}");
                    }
                    _ => {}
                }
            }
    ...
    // Restore the pre-termcmp fg pgrp BEFORE termios teardown. Between inner
    // shell death and this point, a mirror tick may have pointed the outer
    // tty at a dead pgrp, making US background — then any termios call
    ...
    if let Some(orig) = initial_outer_fg {
        ...
        crate::topology::ForegroundMirror::restore_fg(outer_fd, orig);
    }
...
fn wait_with_timeout(child: &mut std::process::Child, deadline: Duration) -> i32 {
...
impl PrivateOscFilter {
    ...
    /// never sees them.
    ///
    /// 7770 is the legacy raw buffer (deprecated; parser still accepts it
    /// with a one-shot warning) and 7772 is the percent-encoded buffer
    /// report. 7771 is the prompt-boundary fallback (defined inline in
    /// `parser`'s performer and emitted by `shell/termcmp.zsh`; see
    /// `docs/ARCHITECTURE.md`). 7773 is the env snapshot (see
    /// `docs/ARCHITECTURE.md`). 7774 is the runtime diagnostic frame.
...
/// shell-native) and the on-demand Ask AI provider.
type BuiltProviders = (
...
pub(crate) fn build_providers(
    ...
    // Shell-native completion providers (pure live, no cache).
    if config.suggest.providers.shell_completions {
        match shell_kind {
            ShellKind::Fish => providers.push(std::sync::Arc::new(
                crate::shell_completion::FishCompletionProvider::new(config.suggest.max_results),
            )),
            ShellKind::Zsh => providers.push(std::sync::Arc::new(
                crate::shell_completion::ZshCompletionProvider::new(config.suggest.max_results),
            )),
            _ => {}
        }
    }

    (providers, ask_ai_provider)
}
...
mod tests {
    ...
    fn parser_with_buffer_and_size(
        ...
        let parser = Arc::new(Mutex::new(TerminalParser::new(rows, cols)));
        // Emit OSC 133;A to set in_prompt = true so trigger() passes the prompt gate
        parser.lock().unwrap().process_bytes(b"\x1b]133;A\x07");
    ...
    fn spawn_child(argv: &[&str]) -> SpawnedTestChild {
        let child = std::process::Command::new(argv[0])
            ...
            .spawn()
            .expect("spawn");
        SpawnedTestChild { child }
    }

    #[test]
    fn wait_with_timeout_returns_before_deadline_for_live_child() {
        let mut spawned = spawn_child(&["sleep", "30"]);
        let pid_before = spawned.child.id();

        let start = std::time::Instant::now();
        let code = wait_with_timeout(&mut spawned.child, Duration::from_millis(200));
    ...
    fn wait_with_timeout_returns_exit_code_of_already_exited_child() {
        ...
        let code = wait_with_timeout(&mut spawned.child, Duration::from_millis(500));
    ...
    fn build_providers_returns_no_llm_when_ai_disabled() {
        ...
        let (providers, ask_ai) = build_providers(&config, ShellKind::Bash);
