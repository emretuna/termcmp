...
/// The on-disk theme file backing `theme_name`, if any:
/// `<watch_dir>/themes/<name>.toml` (empty names resolve to `dark`).
/// Returns `None` for embedded built-ins with no user file — nothing to
/// watch. Follows theme switches by re-deriving after every reload.
/// Extracted for unit testing.
fn active_theme_file(watch_dir: &Path, theme_name: &str) -> Option<PathBuf> {
    let name = if theme_name.is_empty() {
        "dark"
    } else {
        theme_name
    };
    let path = watch_dir.join("themes").join(format!("{name}.toml"));
    path.is_file().then_some(path)
}

/// Identity comparison robust to symlinked temp dirs (macOS `/var` →
/// `/private/var`): canonicalize, falling back to the raw paths.
...
/// Whether `path` is a theme file (`<...>/themes/*.toml`) regardless of which
/// theme is active. Used only for file-creation events: a brand-new file was
/// never the active file, so creation — but not later edits — matches broadly.
...
pub fn spawn_config_watcher(
    ...
    // The themes dir is watched so theme edits hot-reload, but only the
    // *active* theme file triggers a reload (tracked below, refreshed after
    // every reload since a config edit can switch themes). Best-effort: a
    // missing dir is fine (no custom themes installed).
    let themes_dir = watch_dir.join("themes");
    ...
    // Seed the active theme file from the current config; `None` when the
    // theme is an embedded built-in with no file on disk.
    let watched_theme: Option<PathBuf> = TermcmpConfig::load(Some(config_path.as_path()))
        .ok()
        .and_then(|c| active_theme_file(&watch_dir, &c.theme.name));
    ...
    let join = tokio::task::spawn_blocking(move || {
        ...
        let mut watched_theme = watched_theme;
        ...
        loop {
            ...
            // Reload on config.toml edits and on edits to the *active* theme
            // file; other theme files are ignored. `watched_theme` follows
            // theme switches (refreshed after every successful reload).
            // Creation is the exception: a brand-new theme file was never the
            // active file, so any themes/*.toml creation reloads (the reload
            // re-derives the active file).
            let triggers_reload = match event.kind {
            ...
            match apply_config_reload(&handler, &parser, &config) {
                Ok(()) => {
                    ...
                    // A config edit may have switched themes — follow it so
                    // only the newly active file triggers future reloads.
                    watched_theme = active_theme_file(&watch_dir, &config.theme.name);
                }
                Err(e) => {
...
fn build_popup_theme(
    ...
    Ok(PopupTheme {
        ...
        index_hints_on: parse_style(&resolved.index_hints)
            .map_err(|e| anyhow::anyhow!("invalid theme.index_hints: {e}"))?,
        key_hints_on: parse_style(&resolved.key_hints)
            .map_err(|e| anyhow::anyhow!("invalid theme.key_hints: {e}"))?,
        index_hints,
        key_hints,
        nerd_icons,
    })
...
mod tests {
    ...
    #[test]
    fn active_theme_file_tracks_existing_files() {
        let dir = tempfile::tempdir().expect("tempdir");
        let themes = dir.path().join("themes");
        std::fs::create_dir_all(&themes).expect("themes dir");
        std::fs::write(themes.join("vesper.toml"), "description = \"dim\"\n").expect("theme");
        // Existing file resolves to its path.
        assert_eq!(
            active_theme_file(dir.path(), "vesper"),
            Some(themes.join("vesper.toml"))
        );
        // Missing file → None: embedded built-in, nothing to watch.
        assert_eq!(active_theme_file(dir.path(), "dark"), None);
        // Empty name resolves to dark.
        assert_eq!(active_theme_file(dir.path(), ""), None);
        // Identity helper used by the event filter.
        let vesper = themes.join("vesper.toml");
        assert!(same_file(&vesper, &vesper));
        assert!(!same_file(&vesper, &themes.join("other.toml")));
        // Creation matcher: any themes/*.toml, nothing else.
        assert!(is_theme_file(&themes.join("brand-new.toml")));
        assert!(!is_theme_file(&themes.join("notes.txt")));
        assert!(!is_theme_file(&dir.path().join("other.toml")));
    }
