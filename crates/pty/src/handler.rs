...
fn format_key_event(key: &KeyEvent) -> String {
    match key {
        ...
        KeyEvent::ArrowUp | KeyEvent::ArrowUpSs3 => "Up".to_string(),
        KeyEvent::ArrowDown | KeyEvent::ArrowDownSs3 => "Down".to_string(),
        KeyEvent::ArrowLeft | KeyEvent::ArrowLeftSs3 => "Left".to_string(),
        KeyEvent::ArrowRight | KeyEvent::ArrowRightSs3 => "Right".to_string(),
...
/// Default blocklist for Go TUI apps that don't enter alt screen.
///
/// These apps keep `in_prompt == true` because they don't emit OSC 133;A/C
/// transitions, so signal-only detection is insufficient. The foreground
...
fn foreground_process_name() -> Option<String> {
    ...
    #[cfg(target_os = "linux")]
    {
        std::fs::read_to_string(format!("/proc/{fg_pgid}/comm"))
            .ok()
            .map(|s| s.trim().to_string())
    }
    #[cfg(target_os = "macos")]
    {
        macos_proc_name(fg_pgid)
    }
...
/// Best-effort executable name for `pid` via libproc's `proc_name`.
#[cfg(target_os = "macos")]
fn macos_proc_name(pid: libc::pid_t) -> Option<String> {
    #[link(name = "proc")]
    extern "C" {
...
/// Check whether a popup is eligible given the current terminal state.
///
/// A popup is eligible only when the shell is at a prompt (`in_prompt == true`)
/// and not in the alternate screen (`in_alt_screen == false`). This is the
/// single source of truth for popup eligibility — all trigger and merge paths
/// must consult this before rendering. Absence of OSC 133;A / OSC 7771;A means
/// `in_prompt` is false, so the popup stays inactive until the shell reports
/// a prompt boundary.
///
...
///
/// Fish/bash report buffer only on manual Ctrl+/ via OSC 7772; per-keystroke
/// feed is still predicted via `BufferModel`. Zsh uses OSC 7772 as the sole
/// buffer source and skips the model entirely.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum ShellKind {
    Zsh,
    Fish,
...
pub struct InputHandler {
    ...
    max_popup_width: u16,
    /// Adjacent description-box mode (off / on). When `On` an extra
    ...
    /// Fish/bash report buffer only on manual Ctrl+/ via OSC 7772, so
    /// per-keystroke feed is predicted via `BufferModel`.
    shell_kind: ShellKind,
    /// Local keystroke model of the shell command line.
    /// Only populated when no per-keystroke OSC 7772 feed is present.
    ...
    /// Extra process names that suppress popup when at prompt (extends
    /// `DEFAULT_TUI_BLOCKLIST`). Configured via `popup.tui_process_blocklist`.
    tui_process_blocklist: Vec<String>,
    /// Rows below which compact mode activates. From
    /// `config.popup.small_terminal_threshold` ([5,50], default 15).
    small_terminal_threshold: u16,
    /// Max visible suggestions in compact mode. From
    /// `config.popup.small_terminal_max_visible` ([0→5,>10→10], default 5).
...
impl InputHandler {
    pub fn new(terminal_profile: TerminalProfile, shell_kind: ShellKind) -> anyhow::Result<Self> {
        Ok(Self {
            ...
            shell_kind,
            small_terminal_threshold: 15,
            small_terminal_max_visible: 5,
            compact_mode: config::CompactMode::Auto,
    ...
    /// prompt. The `DEFAULT_TUI_BLOCKLIST` is always checked; this extends it
    /// with user-provided names from `popup.tui_process_blocklist`.
    pub fn with_tui_process_blocklist(mut self, blocklist: Vec<String>) -> Self {
        self.tui_process_blocklist = blocklist;
        self
    }

    /// Hot-reload hook for `popup.tui_process_blocklist`.
    pub fn set_tui_process_blocklist(&mut self, blocklist: Vec<String>) {
        self.tui_process_blocklist = blocklist;
    }

    /// Set the compact-mode activation threshold (rows). Stored verbatim —
    /// callers must pass values already clamped at config load time by
    /// `config::TermcmpConfig::normalize`. Corresponds to
    /// `config.popup.small_terminal_threshold`.
    pub fn with_small_terminal_threshold(mut self, threshold: u16) -> Self {
        self.small_terminal_threshold = threshold;
        self
    }

    /// Set the compact-mode visible cap. Corresponds to
    /// `config.popup.small_terminal_max_visible`.
    pub fn with_small_terminal_max_visible(mut self, max: usize) -> Self {
        self.small_terminal_max_visible = max;
        self
    }

    /// Set the compact-mode policy. Corresponds to `config.popup.compact_mode`.
    pub fn with_compact_mode(mut self, mode: config::CompactMode) -> Self {
        self.compact_mode = mode;
        self
    }

    /// Effective blocklist: default entries plus user-configured extras.
    fn effective_tui_blocklist(&self) -> Vec<&str> {
        let mut out: Vec<&str> =
            Vec::with_capacity(DEFAULT_TUI_BLOCKLIST.len() + self.tui_process_blocklist.len());
        out.extend_from_slice(DEFAULT_TUI_BLOCKLIST);
        out.extend(self.tui_process_blocklist.iter().map(|s| s.as_str()));
        out
    }
    ...
    /// call site.
    ///
    /// Re-rank the merged suggestion pool against the live buffer and current word.
    ///
    /// Delegates to `suggest::fuzzy::rank_candidates` which uses two lanes:
    /// full-line candidates (Commands, History, Provider, Llm) match against
    /// the complete `buffer`, while token-local candidates (Filesystem, Env,
    /// SshConfig) match against `current_word`. This fixes the bug where
    /// `supabase back` with cached candidate `supabase backups` would fail to
    /// match if only `current_word` ("back") was used.
    ///
    /// An empty query sorts by source-order, priority, then text — but does NOT
    /// truncate. Truncating on an empty query would evict high-value candidates
    /// from large dynamic pools before a later non-empty re-rank can recover them.
    /// A non-empty query filters and ranks under the engine's configured match
    /// mode, capped at `max_visible * 5`. Frizbee's score order is final — no
    /// post-sort.
    fn rerank_live(
        &self,
        buffer: &str,
        ...
        suggest::fuzzy::rank_candidates_with_order(
            buffer,
            current_word,
            pool,
            self.max_visible * 5,
            self.engine.match_mode(),
            &self.engine.source_order(),
            self.engine.frecency(),
        )
    }
    ...
    pub fn update_config(
        ...
        self.tui_process_blocklist = tui_process_blocklist;
        self.small_terminal_threshold = small_terminal_threshold;
        self.small_terminal_max_visible = small_terminal_max_visible;
    ...
    fn process_key_visible(
        ...
        match key {
            ...
            KeyEvent::Printable(_) | KeyEvent::Backspace => {
                ...
                // Keep the popup visible and re-filter it in place once the shell
    ...
    fn effective_navigation_visible_rows(&self, parser: &Arc<Mutex<TerminalParser>>) -> usize {
        let (screen_rows, cursor_row) = match parser.lock() {
            Ok(p) => {
                let s = p.state();
                (s.screen_dimensions().0, s.cursor_position().0)
            }
            Err(e) => {
                tracing::warn!(
        ...
        let capacity = overlay::popup_content_capacity(
            screen_rows,
            cursor_row,
            self.max_visible,
            self.theme.borders,
            self.theme.index_hints,
            self.theme.key_hints,
            u16::from(!matches!(self.feedback, AsyncFeedback::Idle)),
            self.small_terminal_threshold,
            self.small_terminal_max_visible,
            self.compact_mode,
        );
        if capacity == 0 {
    ...
    pub fn trigger(&mut self, parser: &Arc<Mutex<TerminalParser>>, stdout: &mut dyn Write) {
        ...
        // All providers fire on every trigger (no cache).
        let providers_to_spawn: Vec<Arc<dyn AsyncProvider>> = self.async_providers.clone();
        ...
        let suggestions = match sync_result {
            Ok(r) => r.suggestions,
            Err(e) => {
        ...
        // Zoxide context (`z`/`zi`): the sync pool is already exactly
        // zoxide + history. Shell/LLM generators would only crowd those
        // rows below the visible window before the re-rank, so skip them.
        let zoxide_only = self.engine.is_zoxide_context(&ctx);
        if !suggestions.is_empty() {
            self.replace_suggestions_and_reset_overlay(suggestions);
            self.visible = true;
            // Spawn the selected async providers. All providers fire on
            // every trigger (no cache).
            if !zoxide_only && !providers_to_spawn.is_empty() {
        ...
        } else if !zoxide_only && !providers_to_spawn.is_empty() {
    ...
    pub fn prepare_trigger_with_block(
        ...
        let sync_suggestions = result.map(|r| r.suggestions).unwrap_or_default();
        // Zoxide context (`z`/`zi`): sync pool is already zoxide + history;
        // skip shell/LLM generators so they can't crowd those rows out.
        let spawn_async = !providers_to_spawn.is_empty() && !self.engine.is_zoxide_context(&ctx);
        let needs_block = self.render_block_ms > 0 && spawn_async;
    ...
    pub fn apply_block_result(
        ...
        let all = self.rerank_live(&_live_buffer, &live_word, all);
    ...
    pub fn try_merge_dynamic(
...

        {
            ...
            self.suggestions =
                self.pin_ask_ai(self.rerank_live(&_current_buffer, &current_word, merged));
    ...
    /// Invalidate in-flight async generations so late results cannot render.
    ///
    /// Called by the proxy's stdout task when the parser observes a command-start
    /// marker (OSC 133;C or 7771;C). Bumping `buffer_generation` without updating
    /// `spawned_generation` makes `check_merge_staleness` return `Stale` for any
    /// pending async result, so it gets dropped instead of merged. This prevents
    /// a result spawned before command execution from rendering over the running
    /// command's output.
    pub fn invalidate_inflight_generations(&mut self) {
        self.buffer_generation = self.buffer_generation.wrapping_add(1);
    }
    ...
    fn accept_suggestion_locked(&self, p: &TerminalParser) -> Option<AcceptLocked> {
        ...
        self.engine.frecency().record(&selected.text);
            ...
            || selected.kind == suggest::SuggestionKind::Llm
        {
            ...
            let escaped = match selected.kind {
                suggest::SuggestionKind::FilePath
                | suggest::SuggestionKind::Directory
                | suggest::SuggestionKind::Zoxide => {
...
mod tests {
    ...
    #[test]
    fn test_try_merge_dynamic_empty_query_follows_config_source_order() {
        // Empty-query ranking follows config order: history, shell/provider,
        // then filesystem.
        ...
        let parser = Arc::new(Mutex::new(make_parser_at_prompt()));
        let mut buf = Vec::new();
        ...
        assert_eq!(
            ...
            "empty-query ordering must match config source order: {:?}",
            handler.suggestions,
        );
    }
    ...
    fn test_trigger_active_after_alt_screen_exit() {
        ...
        let parser = Arc::new(Mutex::new(parser::TerminalParser::new(24, 80)));
        parser.lock().unwrap().process_bytes(b"\x1b]133;A\x07"); // Set in_prompt = true
        parser.lock().unwrap().process_bytes(b"\x1b[?1049h");
    ...
    /// Test helper: create a parser with `in_prompt()` set to true.
    /// Required for tests that call `try_merge_dynamic()` or `apply_block_result()`
    /// because the eligibility gate checks `popup_eligible()`.
    fn make_parser_at_prompt() -> parser::TerminalParser {
        let mut p = parser::TerminalParser::new(24, 80);
        p.process_bytes(b"\x1b]133;A\x07"); // OSC 133;A sets in_prompt = true
        p
    }
    ...
    fn test_page_down_uses_effective_popup_height_on_short_terminal() {
        ...
        assert!(result.forward_bytes().is_empty());
        assert_eq!(handler.overlay.selected, Some(6));
        ...
        assert!(selected < handler.overlay.scroll_offset + 1);
    }
    ...
    fn test_page_up_uses_effective_popup_height_on_short_terminal() {
        ...
        assert!(result.forward_bytes().is_empty());
        assert_eq!(handler.overlay.selected, Some(7));
        assert_eq!(handler.overlay.scroll_offset, 7);
        let selected = handler.overlay.selected.unwrap();
        assert!(handler.overlay.scroll_offset <= selected);
        assert!(selected < handler.overlay.scroll_offset + 1);
    }
    ...
    #[test]
    fn test_effective_navigation_visible_rows_agrees_with_below_cursor_capacity() {
        // max_visible=10, small_terminal_max_visible=5, threshold=15, Auto.
        // In compact mode navigation must cap to what renders below the cursor
        // (top/bottom pads are 1 each: index_hints + key_hints). When there is
        // no room below at all, the popup suppresses and navigation falls back
        // to a non-zero guard so move_up/move_down never underflow the list.
        let handler = make_handler();

        // Compact pane (10 < 15): 60 cols wide enough to be unaffected by width.
        // Cursor at row 8 → space_below=1, minus 2 pads = 0 → suppressed → fallback.
        let parser = Arc::new(Mutex::new(parser::TerminalParser::new(10, 80)));
        {
            let mut p = parser.lock().unwrap();
            p.process_bytes(b"\x1b[9;1H"); // row 8 (0-indexed)
        }
        assert_eq!(
            handler.effective_navigation_visible_rows(&parser),
            handler.max_visible.max(1),
            "no room below cursor: suppressed render falls back to max_visible"
        );

        // Cursor at row 5 → space_below=4, minus 2 pads = 2 content rows.
        let parser = Arc::new(Mutex::new(parser::TerminalParser::new(10, 80)));
        {
            let mut p = parser.lock().unwrap();
            p.process_bytes(b"\x1b[6;1H"); // row 5 (0-indexed)
        }
        assert_eq!(
            handler.effective_navigation_visible_rows(&parser),
            2,
            "navigation must cap to the 2 rows that fit below the cursor"
        );

        // Cursor at row 2 → 7 rows below, minus 2 pads = 5 → small-max cap.
        let parser = Arc::new(Mutex::new(parser::TerminalParser::new(10, 80)));
        {
            let mut p = parser.lock().unwrap();
            p.process_bytes(b"\x1b[3;1H"); // row 2 (0-indexed)
        }
        assert_eq!(
            handler.effective_navigation_visible_rows(&parser),
            5,
            "compact mode caps at small_terminal_max_visible"
        );

        // Non-compact pane (24 >= 15): full capacity, cursor position irrelevant.
        let parser = Arc::new(Mutex::new(parser::TerminalParser::new(24, 80)));
        {
            let mut p = parser.lock().unwrap();
            p.process_bytes(b"\x1b[11;1H"); // row 10 (0-indexed)
        }
        assert_eq!(
            handler.effective_navigation_visible_rows(&parser),
            handler.max_visible,
            "non-compact pane must keep full max_visible capacity"
        );
    }
    ...
    #[test]
    fn test_update_config_on_to_on_with_new_size_does_not_stage_cleanup() {
        let mut handler = make_selected_handler(command_suggestion(
            "checkout",
            Some("long description already visible in the detail box"),
        ))
        .with_description_box(DescriptionBoxMode::On, 60, 5, 0);
        handler.last_detail_layout = Some(DetailLayout {
            start_row: 5,
            start_col: 24,
            width: 30,
        ...
        assert!(
            cleanup.is_empty(),
            "On→On reload must not emit any cleanup bytes: {:?}",
            String::from_utf8_lossy(&cleanup)
        );
        assert!(
            handler.last_detail_layout.is_some(),
            "On→On must keep the committed detail layout — no clear is needed",
        );
        let ticket = handler.overlay_write_ticket();
        assert!(
            ticket.cleanup_token.is_none(),
            "On→On must not stage an overlay cleanup token",
        );
    ...
    fn test_render_at_clears_scrolled_old_detail_when_new_selection_has_no_detail() {
        ...
        let additional_scroll = popup_additional_scroll_deficit(
            ...
            handler.small_terminal_threshold,
            handler.small_terminal_max_visible,
            handler.compact_mode,
        );
        assert!(
            additional_scroll > 0,
    ...
    #[test]
    fn test_render_at_renders_on_description_box_for_long_description() {
    ...
    async fn test_detail_box_selection_debounce_holds_then_catches_up() {
        ...
        .with_popup_widths(20, 40)
        .with_description_box(DescriptionBoxMode::On, 60, 5, 200);
        handler.overlay.selected = Some(0);
    ...
    async fn test_detail_box_debounce_wakeup_notifies_and_redraws_selection() {
        ...
        .with_popup_widths(20, 40)
        .with_description_box(DescriptionBoxMode::On, 60, 5, 5);
        handler.overlay.selected = Some(0);
    ...
    fn test_render_at_uses_runtime_popup_and_detail_size_knobs() {
        ...
        let mut detail_handler =
            make_selected_handler(command_suggestion("checkout", Some(detail_desc)))
                .with_popup_widths(20, 40)
                .with_description_box(DescriptionBoxMode::On, 30, 2, 0);
    ...
    fn test_apply_block_result_replacement_resets_detail_debounce_state() {
        let mut handler = make_handler().with_description_box(DescriptionBoxMode::On, 60, 5, 200);
    ...
    fn test_render_for_detail_redraw_noop_when_invisible() {
        let mut handler = make_handler().with_description_box(DescriptionBoxMode::On, 60, 5, 0);
        let parser = Arc::new(Mutex::new(parser::TerminalParser::new(24, 80)));
        let mut buf = Vec::new();
    ...
    #[test]
    fn test_tui_blocklist_suppresses_omp() {
        // DEFAULT_TUI_BLOCKLIST must contain "omp" (Go TUI that doesn't use alt screen)
        assert!(
            DEFAULT_TUI_BLOCKLIST.contains(&"omp"),
            "omp must be in default blocklist"
        );
        // Direct blocklist check: process name "omp" should be considered blocked
        let blocklist: &[&str] = &["omp"];
        assert!(
            blocklist.contains(&"omp"),
            "process name omp must be blocked"
        );
        // Also verify that DEFAULT_TUI_BLOCKLIST blocks "omp" via helper
        let is_blocked = DEFAULT_TUI_BLOCKLIST.contains(&"omp");
        assert!(is_blocked, "omp should be suppressed by blocklist");
        // Test the effective blocklist helper on InputHandler
        let handler = make_handler();
        let eff = handler.effective_tui_blocklist();
        assert!(eff.contains(&"omp"), "effective blocklist must include omp");
    }

    #[test]
    fn test_tui_blocklist_allows_zsh() {
        assert!(
            !DEFAULT_TUI_BLOCKLIST.contains(&"zsh"),
            "zsh must not be in default blocklist"
        );
        let blocklist: &[&str] = DEFAULT_TUI_BLOCKLIST;
        ...
        let handler = make_handler();
        let eff = handler.effective_tui_blocklist();
        assert!(!eff.contains(&"zsh"));
    }

    #[test]
    fn test_tui_blocklist_empty_allows_all() {
        let empty: &[&str] = &[];
        ...
        let handler_empty = make_handler().with_tui_process_blocklist(vec![]);
        let eff = handler_empty.effective_tui_blocklist();
        // effective with empty custom still contains defaults, but empty slice test above proves semantics
        assert!(
            eff.contains(&"omp"),
    ...
    async fn test_trigger_skips_async_providers_for_zoxide_context() {
        // `z` args complete from zoxide + history only: even with async
        // providers configured, no generator may spawn (they would crowd
        // zoxide rows below the visible window before the re-rank).
        ...
        let mut handler = make_handler().with_async_provider(Arc::new(PendingProvider));
        let parser = Arc::new(Mutex::new(parser::TerminalParser::new(24, 80)));
        parser.lock().unwrap().process_bytes(b"\x1b]133;A\x07"); // Set in_prompt = true
        let buffer = "z qqqzzz_no_match_12345".to_string();
        let cursor = buffer.chars().count();
        parser
            .lock()
            .unwrap()
            .state_mut()
            .predict_command_buffer(buffer, cursor);
        let mut buf = Vec::new();
        handler.trigger(&parser, &mut buf);
        assert!(
            handler.dynamic_rx.is_none() && handler.dynamic_task.is_none(),
            "zoxide context must not spawn async providers"
        );
    }

    #[tokio::test]
    async fn test_trigger_spawns_async_providers_for_plain_context() {
        // Control: a non-z empty-sync buffer still spawns async providers.
        use std::future::{pending, Future};
        struct PendingProvider;
        impl AsyncProvider for PendingProvider {
            fn name(&self) -> &'static str {
        ...
        let parser = Arc::new(Mutex::new(parser::TerminalParser::new(24, 80)));
        parser.lock().unwrap().process_bytes(b"\x1b]133;A\x07"); // Set in_prompt = true
        let buffer = "git qqqzzz_no_match_12345".to_string();
        let cursor = buffer.chars().count();
        parser
            .lock()
            .unwrap()
        ...
        handler.trigger(&parser, &mut buf);
        assert!(
            handler.dynamic_rx.is_some() || handler.dynamic_task.is_some(),
            "plain context must still spawn async providers"
        );
    }
}
