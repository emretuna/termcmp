//! Pure keystroke-driven buffer model for shells without per-keystroke feed.
//!
//! Fish/bash report buffer only on manual Ctrl+/ via OSC 7772; per-keystroke
//! feed is still predicted (zsh's shell integration feeds OSC 7772). This
//! model reconstructs the command line by tracking keystrokes locally. The
//! proxy feeds `KeyEvent`s from the input parser through
//! `BufferModel::apply_key`, then pushes the resulting buffer into the same
//! `set_command_buffer` -> `buffer_dirty` -> debounce/trigger pipeline that
//! zsh's OSC 7772 feeds.
//!
...
///
/// Only populated when no per-keystroke OSC 7772 feed is present (fish/bash
/// typing; zsh skips the model). When the model is active, `apply_key` is
/// called for every keystroke before the popup dispatch so the buffer is
/// available for suggestion triggering.
...
impl BufferModel {
    ...
    pub fn apply_key(&mut self, key: &KeyEvent) -> bool {
        match key {
            ...
            KeyEvent::ArrowLeft | KeyEvent::ArrowLeftSs3 => {
                let old = self.cursor;
                self.cursor = self.cursor.saturating_sub(1);
                old != self.cursor
            }
            KeyEvent::ArrowRight | KeyEvent::ArrowRightSs3 => {
