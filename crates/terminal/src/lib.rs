...
impl TerminalProfile {
    ...
    pub fn detect() -> Self {
        ...
        // Inside tmux, session env vars reflect the client that *created* the
        // session (stale once a different terminal re-attaches). Resolve the
        // current client's actual TERM instead; fall back to the leaked-env
        // order in `detect_from_env` only when tmux is unresolvable or the
        // TERM isn't distinctive enough to map to a known emulator.
        if in_tmux {
...
/// Map a terminal's `$TERM` to a known emulator. Only terminals with a
/// distinctive terminfo name are covered; the rest (`xterm-256color` and
/// friends, shared by iTerm2/Terminal.app/VSCode/Zed) fall through to the
/// leaked-session-env order.
fn terminal_from_termname(termname: &str) -> Option<Terminal> {
    match termname {
        "xterm-kitty" | "kitty" => Some(Terminal::Kitty),
        "xterm-ghostty" | "ghostty" => Some(Terminal::Ghostty),
...
/// Ask the active tmux server for the *current* client's TERM. Returns None
/// when tmux isn't queryable (not in a session, tmux not on PATH, or the
/// query fails), leaving the caller to fall back to session env vars.
fn tmux_client_termname() -> Option<String> {
    ...
    let term = String::from_utf8_lossy(&output.stdout).trim().to_string();
    if term.is_empty() {
        None
    } else {
        Some(term)
...
mod tests {
    ...
    #[test]
    fn test_terminal_from_termname() {
        assert_eq!(terminal_from_termname("xterm-kitty"), Some(Terminal::Kitty));
        assert_eq!(
            terminal_from_termname("xterm-ghostty"),
            Some(Terminal::Ghostty)
        );
        assert_eq!(terminal_from_termname("wezterm"), Some(Terminal::WezTerm));
        assert_eq!(
            terminal_from_termname("alacritty"),
            Some(Terminal::Alacritty)
        );
        assert_eq!(terminal_from_termname("xterm-256color"), None);
        assert_eq!(terminal_from_termname("screen-256color"), None);
    }
