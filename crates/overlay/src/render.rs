...
pub struct PopupTheme {
    ...
    pub index_hints_on: Vec<u8>,
    pub key_hints_on: Vec<u8>,
...
/// Shared content-row budget used by painting, viewport scrolling, and
/// navigation. The caller supplies the rows reserved by borders/hints and
#[allow(clippy::too_many_arguments)]
pub fn popup_content_capacity(
    screen_rows: u16,
    cursor_row: u16,
    max_visible: usize,
    borders: bool,
    ...
    let top_pad = u16::from(borders || index_hint);
    let bottom_pad = u16::from(borders || key_hint);
    if is_compact {
        layout::content_capacity_below(
            screen_rows,
            cursor_row,
            max_visible,
            top_pad,
        ...
        layout::content_capacity(screen_rows, max_visible, top_pad, bottom_pad, feedback_rows)
    }
}
...
pub fn popup_additional_scroll_deficit(
    ...
    let _ = small_terminal_max_visible;
    // Compact mode disables viewport scrolling entirely to keep prompt visible.
    if is_compact {
        return 0;
    }
    ...
    let loading_extra_deficit: u16 = u16::from(feedback.reserves_row());
    let effective_max = layout::content_capacity(
        screen_rows,
        max_visible,
        top_pad,
        bottom_pad,
        loading_extra_deficit,
    );
    if effective_max == 0 {
        return 0;
    }
...
pub fn render_popup(
    ...
    crate::sync_frame::with_overlay_update_frame(buf, profile, |buf| {
        layout = Some(render_popup_unframed(
            ...
            feedback,
            hints,
            small_terminal_threshold,
            small_terminal_max_visible,
            compact_mode,
        ));
    });
...
pub fn render_popup_unframed(
    ...
    let capped_max_visible = if is_compact {
        max_visible.min(small_terminal_max_visible)
    } else {
        max_visible
    };
    ...
    // Compact mode never scrolls, so its content budget is bounded by the
    // room below the (adjusted) cursor, not the full screen height — this is
    // what keeps the popup below the prompt on a short pane instead of
    // rendering above it.
    ...
    if effective_max == 0 {
        ...
        eprintln!("termcmp: popup suppressed — no room below cursor ({screen_rows} rows)");
        return PopupLayout {
            start_row: 0,
            start_col: 0,
            width: 0,
    ...
    // The shared capacity reserves the feedback row when active.
    let space_below = screen_rows.saturating_sub(adj_cursor_row.saturating_add(1));
    let visible_count = suggestions.len().min(effective_max) as u16;
    let total_height_needed = visible_count + top_pad + bottom_pad + loading_extra_deficit;
    let new_deficit = if is_compact {
        0
    } else {
        total_height_needed.saturating_sub(space_below)
    };
    let total_deficit = if is_compact {
        effective_prior
    } else {
        effective_prior.saturating_add(new_deficit)
    };
    ...
    if top_pad > 0 {
        ...
        if theme.borders {
            ...
            if let Some((idx, total)) = hints.index {
                ...
                if !theme.index_hints_on.is_empty() {
                    ansi::reset(buf);
                    emit_bg(buf, theme);
                    buf.extend_from_slice(&theme.index_hints_on);
                }
    ...
    if let (Some(bbr), 0) = (bottom_border_row, loading_extra) {
        if theme.borders {
            ...
            if hint_cols <= content_width as usize {
                ...
                if !theme.key_hints_on.is_empty() {
                    ansi::reset(buf);
                    emit_bg(buf, theme);
                    buf.extend_from_slice(&theme.key_hints_on);
                }
                buf.extend_from_slice(label.as_bytes());
                ansi::reset(buf);
                emit_bg(buf, theme);
...
fn render_feedback_only_popup_unframed(
    ...
    let new_deficit = if is_compact {
        ...
        base_height.saturating_sub(space_below)
    };
    let total_deficit = if is_compact {
        effective_prior
    } else {
    ...
    let final_cursor_row = cursor_row.saturating_sub(total_deficit);
    // Compact mode never scrolls; if there isn't room for even the indicator
    // row below the cursor, suppress it — rendering above would cover prior
    // output, which is exactly what we never do.
    if is_compact && space_below < base_height {
        return PopupLayout {
            start_row: 0,
            start_col: 0,
            width: 0,
...
mod tests {
    ...
    fn test_render_small_terminal_caps_height() {
        ...
        // No room below the cursor on a 6-row screen (cursor at row 4 leaves 1
        // row, but borders need 2): compact mode suppresses rather than
        // rendering above the prompt.
        assert_eq!(layout.height, 0, "no room below cursor — popup suppressed");
        assert_eq!(layout.start_row, 0);

        // One row higher there is room for 2 content rows (6-(1+1)-2 borders).
        let mut buf2 = Vec::new();
        ...
        assert_eq!(layout2.height, 4, "2 content rows + 2 borders");
        assert_eq!(layout2.start_row, 2, "renders below cursor, never above");
    }
    ...
    fn test_render_adj_row_never_underflows() {
        ...
        // final_cursor = 0, start_row = 1 (non-compact, scroll enabled)
    ...
    #[test]
    fn kind_icon_returns_zoxide_glyph_and_fallback() {
        assert_eq!(kind_icon(SuggestionKind::Zoxide, true), '\u{F0B07}');
        assert_eq!(kind_icon(SuggestionKind::Zoxide, false), 'z');
    }
    ...
    #[test]
    fn hint_styles_wrap_header_and_footer_labels() {
        let mut buf = Vec::new();
        let suggestions = make_suggestions();
        let state = OverlayState::new();
        let theme = PopupTheme {
            index_hints_on: parse_style("bold").unwrap(),
            key_hints_on: parse_style("fg:196").unwrap(),
            ..bordered_theme()
        };
        let hints = PopupHints {
            index: Some((1, 3)),
        ...
        let output = String::from_utf8_lossy(&buf);
        assert!(
            output.contains("\x1b[1m1/3"),
            "header count must carry the index_hints style: {output}"
        );
        assert!(
            output.contains("\x1b[38;5;196m <Tab> Accept"),
            "footer hint must carry the key_hints style: {output}"
        );
    }
