...
use crate::zoxide::ZoxideProvider;
...
impl SuggestionEngine {
    pub fn new(shell: ShellFamily) -> Result<Self> {
        Ok(Self {
            ...
            zoxide_provider: ZoxideProvider::new(fuzzy::DEFAULT_MAX_RESULTS),
            history_provider: HistoryProvider::load(DEFAULT_MAX_HISTORY_ENTRIES, shell),
    ...
    /// The frecency store for replaying previously accepted suggestions.
    /// Read by the PTY handler so its live re-rank can apply frecency boosts.
    pub fn frecency(&self) -> &crate::frecency::FrecencyStore {
        &self.frecency
    }
    /// Snapshot executable names used by command suggestions.
    pub fn command_names(&self) -> Vec<String> {
        self.commands_provider.command_names()
    }

    /// Snapshot aliases for cache warmup and command candidate construction.
    pub fn alias_entries(&self) -> Vec<(String, crate::alias::AliasEntry)> {
        self.alias_map.entries()
    }
    ...
    pub fn suggest_sync_with_env(
        ...
        match context {
            ...
            Context::UnspeccedArg => {
                // `z`/`zi` take only a directory keyword: the pool is zoxide
                // + history. Filesystem/env/ssh rows would crowd zoxide below
                // the visible window (source order ranks them first), so they
                // are skipped for the zoxide context only.
                if self.is_zoxide_context(ctx) {
                    let mut candidates = Vec::new();
                    if self.config().providers_filesystem {
                        let shell_path = shell_env
                            .as_ref()
                            .and_then(|env| env.get("PATH"))
                            .map(String::as_str);
                        self.extend_with_zoxide_dirs(ctx, cwd, &mut candidates, shell_path);
                    }
                    Ok(self.suggest_rank_only(ctx, cwd, buffer, candidates))
                } else {
                    // No spec — fall back to filesystem + history + situational
                    // injections, including zoxide-ranked directories for z/zi.
                    let mut candidates = Vec::new();
    ...
    /// True when completing an argument to the zoxide jump command (`z`/`zi`
    /// or its resolved shell function/alias). The caller owns a directory
    /// keyword, so only zoxide + history candidates apply.
    pub fn is_zoxide_context(&self, ctx: &CommandContext) -> bool {
        if ctx.word_index == 0 || ctx.is_flag {
            return false;
        }
        match expand_alias_head(ctx, &self.alias_map) {
            Some(command) => crate::zoxide::is_zoxide_command(command.as_ref()),
            None => false,
        }
    }

    /// Rank `candidates` (plus capped history) with no filesystem injection.
    /// Zoxide-context pools use this so directory rows aren't crowded out.
    /// History entries duplicating a zoxide candidate's text are dropped —
    /// in `z` context every directory row comes from the zoxide provider
    /// with `kind == Zoxide`, never as a History-kind double.
    fn suggest_rank_only(
        &self,
        ctx: &CommandContext,
        cwd: &Path,
        buffer: &str,
        ...
        let zoxide_texts: HashSet<String> = candidates.iter().map(|s| s.text.clone()).collect();
        SyncResult {
            suggestions: self.rank_with_history(
                ctx,
                cwd,
                buffer,
                candidates,
    ...
    fn suggest_command_position(
        ...
        if cfg.providers_commands {
            match self.commands_provider.provide(ctx, cwd) {
                Ok(cmds) => {
                    command_names.extend(cmds.iter().map(|s| s.text.clone()));
        ...
        if cfg.providers_commands {
            for (name, entry) in self.alias_map.entries() {
                if name.is_empty() || name.starts_with('-') || !command_names.insert(name.clone()) {
                    continue;
                }
        ...
        SyncResult {
            suggestions: self.rank_with_history(ctx, cwd, buffer, candidates, true, None),
        }
    }
    ...
    fn extend_with_zoxide_dirs(
        ...
        tracing::debug!(
            "extend_with_zoxide_dirs: word_index={}, is_flag={}, command={:?}",
            ctx.word_index,
            ctx.is_flag,
            ctx.command
        );
        ...
        if ctx.word_index == 0 || ctx.is_flag {
            tracing::debug!("extend_with_zoxide_dirs: early return (word_index=0 or is_flag)");
            return;
        }
        let Some(command) = expand_alias_head(ctx, &self.alias_map) else {
            tracing::debug!("extend_with_zoxide_dirs: expand_alias_head returned None");
            return;
        };
        tracing::debug!(
            "extend_with_zoxide_dirs: resolved command={:?}, current_word={:?}",
            command,
            ctx.current_word
        );
        match self
            .zoxide_provider
            .provide_for_command(ctx, cwd, command.as_ref(), shell_path)
        {
            Ok(paths) => {
                tracing::debug!("extend_with_zoxide_dirs: got {} paths", paths.len());
                candidates.extend(paths);
            }
            Err(e) => tracing::debug!("zoxide provider unavailable: {e}"),
        }
    }
    ...
    /// frizbee pass. History entries are appended to the pool (capped).
    ...
    fn rank_with_history(
        ...
        exclude_history_texts: Option<&HashSet<String>>,
    ) -> Vec<Suggestion> {
        let cfg = self.config();
        ...
        if include_history && cfg.max_history_results > 0 && !ctx.in_redirect && !ctx.is_flag {
            match self.history_provider.provide(ctx, cwd) {
                Ok(mut h) => {
                    ...
                    if let Some(exclude) = exclude_history_texts {
                        h.retain(|s| !exclude.contains(&s.text));
                    }
                    candidates.extend(h);
                }
        ...
        fuzzy::rank_candidates_with_order(
            buffer,
            &ctx.current_word,
            candidates,
            cfg.max_results,
...
mod tests {
    ...
    #[test]
    fn test_command_position_includes_aliases_without_duplicates() {
        let engine = make_engine().with_aliases(HashMap::from([
            (
                "gco".to_string(),
                crate::alias::AliasEntry {
                    tokens: vec!["git".to_string(), "checkout".to_string()],
                    description: Some("checkout".to_string()),
                },
            ),
            (
                "git".to_string(),
                crate::alias::AliasEntry::new(vec!["git".to_string()]),
            ),
        ]));
        let ctx = make_ctx(None, vec![], "gco", 0);
        let results = engine.suggest_sync(&ctx, Path::new("/tmp"), "gco").unwrap();
        let gco: Vec<_> = results.iter().filter(|s| s.text == "gco").collect();
        assert_eq!(gco.len(), 1);
        assert_eq!(gco[0].description.as_deref(), Some("checkout"));
    }
    ...
    fn test_source_order_groups_sources() {
        ...
        let results =
            engine.rank_with_history(&ctx, Path::new("/tmp"), "git", candidates, true, None);
    ...
    fn engine_substring_mode_drops_subsequence_only_candidates() {
        ...
        let results =
            engine.rank_with_history(&ctx, Path::new("/tmp"), "git cl", candidates, false, None);
        let texts: Vec<&str> = results.iter().map(|s| s.text.as_str()).collect();
        assert!(
            texts.contains(&"git clone"),
            "contiguous 'cl' match must survive: {texts:?}"
        );
        assert!(
            !texts.contains(&"git calendar"),
    ...
    fn engine_history_skipped_in_redirect_context() {
        ...
        let results =
            engine.rank_with_history(&ctx, Path::new("/tmp"), "echo > ", candidates, true, None);
    ...
    fn engine_history_no_match_preserves_full_candidate_budget() {
        ...
        let candidates = (0..10)
            .map(|i| Suggestion {
                text: format!("git checkout flag{i}"),
                kind: SuggestionKind::Flag,
                source: SuggestionSource::Provider,
                ..Default::default()
            })
    ...
    #[test]
    fn zoxide_context_skips_filesystem_noise() {
        // `z co` must complete from zoxide + history only: planted local
        // files matching "co" appear for `cd` but never for `z`.
        let temp = tempfile::tempdir().unwrap();
        std::fs::write(temp.path().join("code-notes.txt"), "x").unwrap();
        std::fs::write(temp.path().join("taco.md"), "x").unwrap();
        let history = HistoryProvider::from_entries(vec!["z Code".into(), "git push".into()]);
        let commands = CommandsProvider::from_list(vec!["git".into()]);
        let engine = make_test_engine(history, commands);

        let zctx = make_ctx(Some("z"), vec![], "co", 1);
        assert!(engine.is_zoxide_context(&zctx));
        let zres = engine.suggest_sync(&zctx, temp.path(), "z co").unwrap();
        for s in zres.iter() {
            assert!(
                matches!(
                    s.source,
                    SuggestionSource::Zoxide | SuggestionSource::History
                ),
                "z context must be zoxide+history only, got {s:?}"
            );
        }
        assert!(
            zres.iter().any(|s| s.text == "z Code"),
            "history `z Code` must survive the z filter, got {:?}",
            zres.iter().map(|s| &s.text).collect::<Vec<_>>()
        );

        let cctx = make_ctx(Some("cd"), vec![], "co", 1);
        assert!(!engine.is_zoxide_context(&cctx));
        let cres = engine.suggest_sync(&cctx, temp.path(), "cd co").unwrap();
        assert!(
            cres.iter()
                .any(|s| s.source == SuggestionSource::Filesystem),
            "non-z context keeps filesystem rows, got {:?}",
            cres.iter().map(|s| &s.text).collect::<Vec<_>>()
        );
    }

    #[test]
    fn zoxide_context_dedups_history_against_zoxide_text() {
        // A history entry duplicating a zoxide directory row would render a
        // History-kind double with the wrong icon. Dedup keeps exactly one
        // row for the text — the Zoxide one — while unrelated history rows
        // survive untouched.
        let dir_text = "/tmp/termcmp";
        let history = HistoryProvider::from_entries(vec![dir_text.into(), "git push".into()]);
        let engine = make_test_engine(history, CommandsProvider::from_list(vec!["z".into()]));
        let candidates = vec![Suggestion {
            text: dir_text.to_string(),
            description: Some("zoxide directory".to_string()),
            kind: SuggestionKind::Zoxide,
            source: SuggestionSource::Zoxide,
            ..Default::default()
        }];
        let ctx = make_ctx(Some("z"), vec![], "", 1);
        let res = engine.suggest_rank_only(&ctx, Path::new("/tmp"), "", candidates);
        let matches: Vec<_> = res
            .suggestions
            .iter()
            .filter(|s| s.text == dir_text)
            .collect();
        assert_eq!(
            matches.len(),
            1,
            "duplicated text must collapse to one row, got {:?}",
            res.suggestions.iter().map(|s| &s.text).collect::<Vec<_>>()
        );
        assert_eq!(matches[0].kind, SuggestionKind::Zoxide);
        assert_eq!(matches[0].source, SuggestionSource::Zoxide);
        assert!(
            res.suggestions
                .iter()
                .any(|s| s.text == "git push" && s.kind == SuggestionKind::History),
            "non-duplicate history rows must survive: {:?}",
            res.suggestions.iter().map(|s| &s.text).collect::<Vec<_>>()
        );
    }
}
