...
impl CommandsProvider {
    ...
    /// Return the scanned executable names for runtime cache warmup.
    pub fn command_names(&self) -> Vec<String> {
        self.commands.iter().map(|name| name.to_string()).collect()
    }
...
mod tests {
    ...
    #[test]
    fn test_command_names_snapshot() {
        let provider = CommandsProvider::from_list(vec!["git".into(), "ls".into()]);
        assert_eq!(provider.command_names(), vec!["git", "ls"]);
    }
