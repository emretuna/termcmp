...
impl HistoryProvider {
    pub fn load(max_entries: usize, shell: crate::ShellFamily) -> Self {
        let path = Self::history_path(shell).ok();
        let (entries, fingerprint) = match &path {
            Some(p) => {
                let fp = FileFingerprint::from_path(p);
                match Self::read_history_from(p, max_entries, shell) {
    ...
    fn refresh_if_stale(&self) {
        ...
        match Self::read_history_from(path, self.max_entries, self.shell) {
            Ok(entries) => {
    ...
    fn read_history_from(
        ...
        Ok(Self::parse_and_dedup(&clean, max_entries, shell))
    }
    ...
    ///   rule 1 or 2 (or both).
    fn history_path(shell: crate::ShellFamily) -> Result<PathBuf> {
        ...
        if let Ok(histfile) = std::env::var("HISTFILE") {
            ...
            match Self::validate_history_path(&candidate, &home) {
                ...
                Err(e) => {
                    tracing::warn!(
                        "ignoring $HISTFILE={histfile:?}: {e}; falling back to shell default"
                    );
                }
            }
        }

        // Shell-specific default paths
        let default = match shell {
            crate::ShellFamily::Fish => home.join(".local/share/fish/fish_history"),
            crate::ShellFamily::Bash => home.join(".bash_history"),
    ...
    fn parse_and_dedup(
        ...
        if shell == crate::ShellFamily::Fish {
            // Fish uses YAML-like format: "- cmd: command\n  when: timestamp"
            // Extract only the "- cmd:" lines
...
mod tests {
    ...
    fn test_refresh_picks_up_new_entries() {
        ...
        let provider = HistoryProvider {
            state: Mutex::new(HistoryState {
                entries: HistoryProvider::parse_and_dedup(
                    "ls\ncd /tmp\n",
                    1000,
                    crate::ShellFamily::Zsh,
                ),
    ...
    fn test_read_history_skips_invalid_utf8_lines() {
        ...
        let entries =
            HistoryProvider::read_history_from(&path, 1000, crate::ShellFamily::Zsh).unwrap();
    ...
    fn test_fish_history_parsing() {
        // Fish uses YAML-like format: "- cmd: command\n  when: timestamp"
        let fish_history = r#"- cmd: git diff
  when: 1747752414
- cmd: herdr server stop
  when: 1747752500
  paths:
    - /tmp
- cmd: cargo build
  when: 1747752600
"#;
        let entries = HistoryProvider::parse_and_dedup(fish_history, 100, crate::ShellFamily::Fish);
        assert_eq!(entries.len(), 3);
        assert_eq!(entries[0], "git diff");
        assert_eq!(entries[1], "herdr server stop");
        assert_eq!(entries[2], "cargo build");
    }

    #[test]
    fn test_fish_history_dedup() {
        // Fish history with duplicate commands should dedup, keeping most recent
        let fish_history = r#"- cmd: git diff
  when: 1747752414
- cmd: cargo build
  when: 1747752500
...
"#;
        ...
        // Most recent occurrence wins, order is most-recent-last
        assert_eq!(entries[0], "cargo build");
        assert_eq!(entries[1], "git diff");
    }
}
