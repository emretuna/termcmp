...
|-------|---------|------------------|
| [`termcmp`](../crates/termcmp/) | Binary entry point, CLI (`clap`), install/uninstall, `config`, `doctor` | pty, config, suggest, terminal, overlay + clap/tokio/anyhow/dirs/tracing/toml/libc/toml_edit/tempfile |
| [`pty`](../crates/pty/) | PTY proxy event loop — spawns shell, multiplexes stdin/stdout with `tokio::select!`, handles SIGWINCH, async provider merge | crossterm/serde/serde_json/dirs/tempfile/parser/buffer/suggest/overlay/config/terminal/llm/notify/libc + portable-pty/tokio |
| [`parser`](../crates/parser/) | VT escape sequence parsing — cursor position, screen dimensions, prompt boundaries (OSC 133 + OSC 7771), CWD (OSC 7), exported env (OSC 7773) | unicode-width/anyhow/tracing + vte |
| [`buffer`](../crates/buffer/) | Command line reconstruction — current command, argument position, pipes, redirects, quotes | |
| [`suggest`](../crates/suggest/) | Suggestion engine — dispatches to providers, fuzzy-ranks with frizbee | buffer/config/tokio/anyhow/serde/tracing/dirs/regex/shlex/tempfile + frizbee/serde_json |
| [`overlay`](../crates/overlay/) | ANSI popup rendering — cursor save/restore, synchronized output, scroll-to-make-room, scrollbar, fuzzy match highlighting | config/suggest/terminal/anyhow/unicode-width |
| [`config`](../crates/config/) | TOML config, keybindings, themes (presets + custom styles), suggestion timeouts | dirs/anyhow/tracing + serde/toml |
...
| [`llm`](../crates/llm/) | OpenAI-compatible chat-completions client powering the AI suggestion and Ask-AI providers | suggest/anyhow/tracing/rustls/regex + reqwest/serde_json (buffer is a normal-dep used only in `#[cfg(test)]` — candidate to scope as dev-dep) |
...
termcmp ──► pty ──► parser
                     ...
                     │                  └─► terminal
                     │                  └─► config
                     │    ──► llm ──► suggest (+ buffer test-only)
```

`parser`, `buffer`, `config`, and `terminal` are leaf crates with no internal dependencies. `suggest` depends on `buffer` and `config`. `overlay` depends on `suggest`, `terminal`, and `config`. `llm` depends on `suggest` (+ `buffer` test-only). `pty` depends on every other crate and ties them all together.
...
`frizbee` is the sole fuzzy matching engine. It supports two match modes — `Fuzzy` (subsequence: `gco` matches `git checkout`) and `Substring` (contiguous: `cl` matches `clone` but not `calendar`) — with smart case sensitivity. The matcher is constructed per query via `frizbee::Matcher::new(query, &config)` and applied via `matcher.match_iter_indices(haystacks.iter())` inside `suggest/src/fuzzy.rs:60/64` (`rank_with_mode`), called per lane by `rank_candidates_with_order:125-175` (Filesystem/Zoxide/Env/SshConfig token-local on `current_word`; Commands/History/Provider/Llm full-line on `buffer`), merged sort by `(SourceOrder rank, priority::effective desc, score desc, text)` + `truncate(max_results)`. For an autocomplete tool running on every keystroke, this keeps ranking well under 1ms for 10,000 candidates.
...
The PTY proxy runs seven concurrent worker tasks plus a main coordination loop:
...
| **Task E** (feedback tick) | `tokio::spawn` | Drives spinner animation for Loading feedback |
| **Task F** (detail redraw) | `tokio::spawn` | Debounced redraw of the description/detail box on selection change |
| **Task G** (mode flash) | `tokio::spawn` | Brief flash notification when match mode toggles (Ctrl+R) |
| **Config watcher** | `spawn_blocking` | Watches `config.toml` for changes and hot-reloads applied settings |
...
Benchmarks use Criterion and live in `parser` and `overlay`. Run with `cargo bench`.
...
- **OSC 7773** — Termcmp's exported environment snapshot (zsh only; fish/bash have no 7773 reporter so `shell_env` stays `None` and env-aware providers degrade), consumed by the proxy and stripped before terminal output
- **OSC 7774** — diagnostic feedback (env_truncated, zle_hook_disabled, Unknown)

Prompt markers are emitted simultaneously by the integration scripts, so the parser can use whichever the terminal supports. Native-OSC133 sets are asymmetric: zsh covers Ghostty/`GHOSTTY_RESOURCES_DIR`/Otty/`KITTY_WINDOW_ID`/`WEZTERM_UNIX_SOCKET`/WezTerm/Rio/`ZED_TERM`/`VSCODE_INJECTION`, while fish+bash cover only Ghostty/Otty/`ZED_TERM`/`VSCODE_INJECTION`. The redundant 7771 on Kitty/WezTerm/Rio under fish/bash is harmless: the parser accepts both, `PrivateOscFilter PRIVATE_CODES 7770..7774` strips 7771 and preserves 133/633.

Without shell integration, features are limited — prompt boundary detection falls back to heuristics, and manual trigger (Ctrl+/) is the only way to invoke completions. Bash scope stays as-is: markers + manual Ctrl+/ (`bind -x`), no `init.bash`, no 7773 env / 7774 diagnostics, `command -v termcmp || return` no-op. Bash has no smoke matrix (`shell:[zsh,fish]`) — known gap.

### Additional Features

**Popup feedback system.** The popup displays transient feedback messages for loading, empty, and error states. Loading feedback includes an animated spinner (when `popup.spinner = true`). Empty feedback appears when no suggestions match. Error feedback shows provider failures (when `popup.show_provider_errors = true`). All feedback auto-dismisses after `popup.feedback_dismiss_ms` (default 1200ms).

**Description/detail box.** When `popup.description_box = "on"`, a wrapped multi-line box renders beside the main popup showing the full description of the selected suggestion. Updates are debounced by `popup.description_box_debounce_ms` (default 80ms) to avoid flicker during rapid navigation. The box falls back to stacked-below or inline-truncated modes when horizontal space is insufficient.

**Match-mode toggle.** Ctrl+R toggles between fuzzy (subsequence) and substring (contiguous) matching at runtime. The active mode flashes briefly in the popup footer. The toggle is bound to `keybindings.toggle_match_mode`.

**Config hot-reload.** A background config watcher (`spawn_blocking`) monitors `~/.config/termcmp/config.toml` for changes. Most settings (theme, keybindings, trigger, popup, suggest, ai) apply immediately without restarting the shell. See [CONFIGURATION.md](CONFIGURATION.md) for the hot-reload table.

**Buffer prediction.** Fish/bash report buffer only on manual Ctrl+/ via OSC 7772; per-keystroke feed is still predicted by replaying keystrokes through a lightweight state machine (`pty/src/predict.rs`). Prediction is less accurate than shell-reported buffers but enables basic completion functionality.
...
(Narrow alphabet prevents `;`/BEL/ESC splitting OSC params.)

OSC 7770 (the prior raw framing) is accepted by the parser as a deprecated
read-only path: the first hit per process logs a one-shot
`tracing::warn!` and subsequent hits drop to `trace!`. The 7770 dispatch
arm is slated for removal in v0.12.0. zsh, fish, and bash emit OSC 7772
(`shell/termcmp.zsh:_tc_report_buffer`, `shell/termcmp.fish:_termcmp_report_buffer`,
`shell/termcmp.bash:134`); OSC 7770 is a parser-only deprecated read path.
