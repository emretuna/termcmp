...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `max_visible` | integer | `10` | Maximum number of suggestions shown at once. `0` resets to default `10`; values above `50` are clamped to `50`. |
...
| `description_box` | string | `"off"` | Adjacent description box mode. `"off"` keeps the legacy inline-truncated behavior. `"on"` renders a wrapped multi-line box next to the main popup for the selected suggestion when the inline description would be hidden or truncated. The box is capped by `description_box_lines` and available rows; short descriptions that already fit don't trigger it. Falls back to a stacked-below box when there's no horizontal room, and to inline truncation when neither fits. |
...
| `small_terminal_threshold` | integer | `15` | Rows below which compact mode activates. When `screen_rows` is less than this value, visible suggestions are capped and scrolling is disabled to keep the prompt visible. Clamped to `[5, 50]`. |
| `small_terminal_max_visible` | integer | `5` | Maximum visible suggestions in compact mode. Capped to `max_visible`. `0` resets to default `5`; values above `10` are clamped to `10`. |
| `compact_mode` | string | `"auto"` | Small terminal handling mode. `"auto"` enables compact logic when `screen_rows < small_terminal_threshold`, `"always"` forces compact mode regardless of size, `"never"` disables it. |
| `tui_process_blocklist` | string array | `[]` | Additional process names that suppress the popup even when at prompt. Extends the built-in blocklist (`omp`, `pi`, `agent`, `btm`, etc.) for Go TUI apps that don't enter alt screen and keep `in_prompt == true`. |
...
compact_mode = "auto"
tui_process_blocklist = []
```

Popup width is content-driven (sized to the longest visible suggestion) and clamped to `[min_width, max_width]`. Descriptions that don't fit are truncated with a single-column ellipsis (`…`). Set `description_box = "on"` to surface a wrapped description when the inline description would be hidden or truncated. The box is capped by `description_box_lines` and available rows without permanently widening the main popup.
...
| `filesystem` | bool | `true` | File and directory completions (also gates zoxide — see below) |
...
**Zoxide coupling:** zoxide has no dedicated flag. Zoxide directories are
offered when `providers.filesystem == true` AND the buffer is a zoxide
context (`z`/`zi`/`__zoxide_z`/`__zoxide_zi`, `suggest/src/zoxide.rs:70-158`,
gated on the engine's zoxide branch via `extend_with_zoxide_dirs`). The
`zoxide` entry in `suggest.order` is rank-only: omitting a valid name sorts
that source last (`usize::MAX`), it does not disable it; unknown names are
dropped with a warning (`normalize:879-909`).

**`shell` order name:** maps to the async `SuggestionSource::Provider` bucket
(fish/zsh native via `pty/src/shell_completion.rs:195/364`, gated on
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `name` | string | `"dark"` (stored as empty, resolves to dark) | Theme name: built-in (`dark`, `light`, `catppuccin`, `material-darker`, `gruvbox`, `nord`, `dracula`, `tokyo-night`) or a custom `themes/<name>.toml` file in the config directory. |
...
| Theme | Selected | Description | Match Highlight | Item Text | Kind Icon | Scrollbar | Border | Feedback Error | Index Hints | Key Hints |
|-------|----------|-------------|-----------------|-----------|-----------|-----------|--------|----------------|-------------|-----------|
| `dark` | `reverse` | `dim` | `bold` | *(none)* | `dim` | `dim` | `dim` | `dim fg:196` | `dim` | `dim` |
| `light` | `fg:#1f2328 bg:#e7edf3 bold` | `fg:#6e7781` | `fg:#cf222e bold` | `fg:#24292f` | `fg:#d0d7de` | `fg:#8c959f` | `fg:#d0d7de` | `fg:#cf222e bold` | `fg:#6e7781` | `fg:#6e7781` |
| `catppuccin` | `fg:#cdd6f4 bg:#585b70 bold` | `fg:#6c7086` | `fg:#f9e2af bold` | *(none)* | `fg:#a6e3a1` | `fg:#585b70` | `fg:#585b70` | `dim fg:#f38ba8` | `fg:#a6adc8` | `fg:#a6adc8` |
| `material-darker` | `fg:#eeffff bg:#424242 bold` | `fg:#616161` | `fg:#ffcb6b bold` | *(none)* | `fg:#c3e88d` | `fg:#616161` | `fg:#616161` | `dim fg:#ff5370` | `fg:#8A8A8A` | `fg:#8A8A8A` |
| `gruvbox` | `fg:#ebdbb2 bg:#504945 bold` | `fg:#928374` | `fg:#fabd2f bold` | *(none)* | `fg:#b8bb26` | `fg:#504945` | `fg:#504945` | `dim fg:#fb4934` | `fg:#a89984` | `fg:#a89984` |
| `nord` | `fg:#d8dee9 bg:#4c566a bold` | `fg:#4c566a` | `fg:#ebcb8b bold` | *(none)* | `fg:#a3be8c` | `fg:#4c566a` | `fg:#4c566a` | `dim fg:#bf616a` | `fg:#81A1C1` | `fg:#81A1C1` |
| `dracula` | `fg:#f8f8f2 bg:#44475a bold` | `fg:#6272a4` | `fg:#f1fa8c bold` | *(none)* | `fg:#50fa7b` | `fg:#44475a` | `fg:#44475a` | `dim fg:#ff5555` | `fg:#6272A4` | `fg:#6272A4` |
| `tokyo-night` | `fg:#c0caf5 bg:#283457 bold` | `fg:#565f89` | `fg:#e0af68 bold` | *(none)* | `fg:#9ece6a` | `fg:#3b4261` | `fg:#3b4261` | `dim fg:#f7768e` | `fg:#737aa2` | `fg:#737aa2` |

All built-in themes except `light` leave `item_text` unstyled (default terminal foreground). `feedback_loading` and `feedback_empty` inherit `description` by default. `index_hints` and `key_hints` also inherit `description` in every built-in theme. `index_hints` styles the header `selected/total` count and `key_hints` styles the footer key helper text; set either to `""` for today's plain unstyled look.

#### Custom Themes

Theme files are the ONLY way to customize styles. Place a TOML file at `~/.config/termcmp/themes/<name>.toml` with any of these keys: `selected`, `description`, `match_highlight`, `item_text`, `kind_icon`, `scrollbar`, `border`, `feedback_loading`, `feedback_empty`, `feedback_error`, `background`, `description_box_background`, `index_hints`, `key_hints`. Omitted keys fall back to the built-in `dark` theme. Set `name = "<name>"` in config.toml to load it.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `redact_secrets` | bool | `true` | Redact API keys, tokens, and passwords from buffer and history before sending to LLM providers. Strips env names matching `*_KEY`, `*_TOKEN`, `*_SECRET`, `*_PASSWORD`, `API_*`, `AUTH_*`, bearer tokens, and prefixes like `sk-`, `ghp_`, `AKIA*`. |
...
api = "openai-chat"  # "openai-chat" or "openai-responses". Any other value (including typos) falls back to `openai-chat`.
...
id = "gpt-4o"
name = "GPT-4o"
max_tokens = 512  # optional per-model override
```

Provider leaves are `base_url`, `api_key`, `api`, `thinking_budget`,
`extra_body`, and the `models` list (each entry: `id`, `name`, optional
...
Termcmp maintains a frecency store that acts as a ranking modifier — it boosts the scores of frequently-accepted suggestions within their original sources so that commands you actually use rise to the top of future completions. It does not create separate suggestion entries or appear as its own source in the popup. Every time you accept a suggestion (via Tab or Enter), the accepted text is recorded with a timestamp, and on subsequent triggers matching candidates from any source receive a score boost proportional to their frecency.

In other words, frecency is invisible to users — it only affects ranking. A frequently-accepted command like `docker compose up` will rank higher than an unfamiliar match even when the fuzzy scores are close, but it still appears under its original source (e.g. History or Shell Completions), not as a separate "Frecency" group.

### Scoring

Each acceptance pushes a timestamp onto the entry (≤32/entry, oldest dropped
first). The per-acceptance weight is `exp(-0.0693·days_ago)` (10-day
half-life), summed over in-window timestamps; `normalize = identity ≤10 else
10+sqrt(total-10)`; `boost = round(score·8.0)` (`BOOST_MULTIPLIER=8.0`),
applied in `suggest/src/fuzzy.rs:72` as
`(frizbee_score + frecency.boost(text)).min(u16::MAX)`. A single fresh accept
is a mild nudge; heavy recent use dominates within a source group.

### Store File Location

```
$XDG_STATE_HOME/termcmp/frecency.json
```

When `XDG_STATE_HOME` is unset, it falls back to:

```
~/.local/state/termcmp/frecency.json
```

### How It Populates

The store builds through normal use. Each time you accept any suggestion, the
accepted text is recorded unconditionally (`accept_suggestion_locked`
records every accepted text regardless of source — ranking-only boost per
design). Matching candidates have their fuzzy match scores boosted by their
frecency score, so familiar commands naturally float to the top of their
source group without creating duplicate entries.

### Resetting the Store

To start fresh, delete the store file and restart termcmp:

```bash
rm "${XDG_STATE_HOME:-$HOME/.local/state}/termcmp/frecency.json"
```

The file is recreated automatically on the next acceptance. No configuration change is needed.

Prune is per-timestamp with a `MAX_HISTORY_DAYS=30.0` cutoff on
load/record/score (not per-entry); eviction removes the `lowest_scoring_key`
when `len>1000` (not oldest-first); per-entry timestamps cap at 32.
