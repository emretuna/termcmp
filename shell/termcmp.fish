...
function _termcmp_urlencode_path
    ...
    for ch in (string split "" -- $input)
        ...
        if string match -qr '^[a-zA-Z0-9._~/-]$' -- $ch
            ...
            set -l cp (printf '%d' "'$ch")
            if test $cp -lt 128
                set encoded "$encoded"(printf '%%%02X' $cp)
            else if test $cp -lt 2048
                set -l b1 (math "192 + floor($cp / 64)")
                set -l b2 (math "128 + ($cp % 64)")
                set encoded "$encoded"(printf '%%%02X%%%02X' $b1 $b2)
            else if test $cp -lt 65536
                set -l b1 (math "224 + floor($cp / 4096)")
                set -l b2 (math "128 + floor(($cp / 64) % 64)")
                set -l b3 (math "128 + ($cp % 64)")
                set encoded "$encoded"(printf '%%%02X%%%02X%%%02X' $b1 $b2 $b3)
            else
                set -l b1 (math "240 + floor($cp / 262144)")
                set -l b2 (math "128 + floor(($cp / 4096) % 64)")
                set -l b3 (math "128 + floor(($cp / 64) % 64)")
                set -l b4 (math "128 + ($cp % 64)")
                set encoded "$encoded"(printf '%%%02X%%%02X%%%02X%%%02X' $b1 $b2 $b3 $b4)
            end
        end
    end
    echo -n $encoded
...
# Report buffer via OSC 7772 (percent-encoded), only when the proxy is active.
...
function _termcmp_report_env
    ...
    # Cheap fingerprint to skip expensive encoding when env unchanged.
    set -l _termcmp_env_count (count (set -x --names))
    set -l _termcmp_fingerprint "$_termcmp_env_count:$PWD:$SHLVL"
    if test "$_termcmp_fingerprint" = "$_TERMCMP_LAST_ENV_FINGERPRINT"
        return
    end
    set -g _TERMCMP_LAST_ENV_FINGERPRINT "$_termcmp_fingerprint"
    ...
    # Sort once and reuse for both loops.
    set -l sorted_names (set -x --names | sort)
