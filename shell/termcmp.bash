...
# Strict alphabet [A-Za-z0-9._~/-]: vte splits OSC params on `;`, so a raw
# semicolon (e.g. `/tmp/foo;bar`) would split the OSC 7 frame mid-path.
_tc_urlencode_path() {
...
# Percent-encode a buffer for OSC 7772 transport. Strict alphabet:
# [a-zA-Z0-9._~/-] plus a literal space pass through, everything else %XX.
...
# Report buffer to proxy via OSC 7772 (percent-encoded) when the proxy is
# active. OSC 7770 is deprecated: raw framing truncates at the first ';'.
_tc_report_buffer() {
    [[ -n "$TERMCMP_ACTIVE" ]] || return
    printf '\e]7772;%d;%s\a' "$READLINE_POINT" "$(_tc_urlencode_buffer "$READLINE_LINE")"
}
