...
# OSC 7771: terminal-agnostic prompt boundary for Termcmp
...
# encoded because vte splits OSC params on it (narrow alphabet prevents splitting).
_tc_urlencode_path() {
...
# space. Differs from _tc_urlencode_path only by allowing space (path
...
# the parser's state machine (narrow alphabet prevents `;`/BEL/ESC splitting).
#
...
typeset -gi _TC_ENV_TOTAL_BUDGET=524288
typeset -gi _TC_ENV_PER_VALUE_CAP=16384

_tc_report_env() {
    [[ -n "$TERMCMP_ACTIVE" ]] || return

    local _tc_fingerprint="${#parameters[(R)*export*]}:${PWD}:${SHLVL}"
    if [[ "$_tc_fingerprint" == "${_TC_LAST_ENV_FINGERPRINT-}" ]]; then
        return
    fi

    local key meta value encoded entry payload="" truncated=0
    local -i used=0
    local -a _tc_exported_keys=(${(ok)parameters[(R)*export*]})
    ...
    _tc_env_emit() {
        ...
        case "$k" in
            TERMCMP_*|_TC_*) return ;;
        esac
        [[ -n "$k" && "$k" != [0-9]* && "$k" != *[^a-zA-Z0-9_]* ]] || return
        meta="${parameters[$k]}"
        [[ "$meta" == *scalar* && "$meta" == *export* ]] || return
        value="${(P)k}"
        entry="$(_tc_urlencode_buffer "${k}=${value}")"
        if (( ${#entry} > _TC_ENV_PER_VALUE_CAP )); then
            truncated=1
            return
        fi
        if (( used + ${#entry} + 3 > _TC_ENV_TOTAL_BUDGET )); then
            truncated=1
            return
        fi
        payload+="${entry}%00"
    ...
    local prefix
    for key in "${_tc_exported_keys[@]}"; do
    ...
    unset -f _tc_env_emit

    if [[ "$payload" == "${_TC_LAST_ENV_PAYLOAD-}" ]]; then
        typeset -g _TC_LAST_ENV_FINGERPRINT="$_tc_fingerprint"
        return
    fi
    typeset -g _TC_LAST_ENV_PAYLOAD="$payload"
    typeset -g _TC_LAST_ENV_FINGERPRINT="$_tc_fingerprint"
    printf '\e]7773;%s\a' "$payload"

    # Truncation diagnostic on OSC 7774. One-shot via _TC_ENV_TRUNCATED_REPORTED.
    if (( truncated )) && [[ -z "${_TC_ENV_TRUNCATED_REPORTED-}" ]]; then
        typeset -g _TC_ENV_TRUNCATED_REPORTED=1
...
_tc_chpwd() {
    printf '\e]7;file://%s%s\a' "$HOST" "$(_tc_urlencode_path "$PWD")"
}

add-zsh-hook chpwd _tc_chpwd
# Also emit on first prompt in case the shell started in a non-default directory
add-zsh-hook precmd _tc_osc7_precmd
_tc_osc7_precmd() {
    printf '\e]7;file://%s%s\a' "$HOST" "$(_tc_urlencode_path "$PWD")"
    # Remove self after first run — chpwd hook handles subsequent changes
    add-zsh-hook -d precmd _tc_osc7_precmd
}
...
#
# Do NOT inline `$BUFFER` raw into the OSC payload (narrow alphabet prevents splitting).
...
_tc_report_buffer() {
    [[ -n "$TERMCMP_ACTIVE" ]] || return
    local encoded
    encoded="$(_tc_urlencode_buffer "$BUFFER")"
...
_tc_install_zle_hook() {
    ...
    if [[ "$current" == "user:_tc_zle_line_pre_redraw" || "$current" == "user:_tc_report_buffer" ]]; then
        return
    fi
    # Three-way dispatch:
    #   1. No widget registered         → direct-install _tc_report_buffer.
    ...
    #      zle_hook_disabled (only when $TERMCMP_ACTIVE is set, so
    ...
    if (( ! ${+widgets[zle-line-pre-redraw]} )); then
        zle -N zle-line-pre-redraw _tc_report_buffer
    elif [[ "${widgets[zle-line-pre-redraw]}" == user:* ]]; then
        local existing="${widgets[zle-line-pre-redraw]}"
        zle -N _tc_orig_zle_line_pre_redraw "${existing#user:}"
        _tc_zle_line_pre_redraw() {
            _tc_report_buffer
            zle _tc_orig_zle_line_pre_redraw -- "$@"
        }
        zle -N zle-line-pre-redraw _tc_zle_line_pre_redraw
    else
        ...
        # emission is gated on $TERMCMP_ACTIVE so we don't leak the
        # frame to a bare terminal when the proxy isn't watching.
        local descriptor="${widgets[zle-line-pre-redraw]}"
        local encoded_desc
        encoded_desc="$(_tc_urlencode_buffer "$descriptor")"
